...
- (void)getDataFromNet {
    /*
     [BiddingDetailModel getDataWithOperation:BIDDING_DETAIL_API parameters:@{} endBlock:^(id model, NSError *error) {
     if (!error) {
...
- (void)sipplierButtonAction:(UIButton *)sender {
    ...
    [BidPickerView showTimeSelectViewWithTitle:@"请选择运营商" dataArr:self.biddingModel.supplieArr selectBlock:^(id model) {
        ...
        [[NetworkHelper shareClient] GET:GET_DRIVER_API parameters:@{@"phoneNumber":[LoginModel shareLoginModel].phone, @"bidCode":tempModel.bidCode, @"supplierCode":detailModel.supplierCode} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)rapButtonAction:(UIButton *)sender {
    ...
    NSDictionary *paraDict = @{@"phoneNumber":[LoginModel shareLoginModel].phone,
