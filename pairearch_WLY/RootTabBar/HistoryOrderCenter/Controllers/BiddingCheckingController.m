...
- (void)loadDataFromNet{
    /*
     [BiddingCheckingModel getDataWithOperation:BIDDINF_CHECKING_API parameters:@{@"phoneNumber":[LoginModel shareLoginModel].phone, @"bidCode":self.bidCode} endBlock:^(id model, NSError *error) {
     if (!error) {
...
- (void)changeBiddingPost {
    ...
    [[NetworkHelper shareClient] POST:CHANGE_BIDDING_API parameters:@{@"phoneNumber":[LoginModel shareLoginModel].phone, @"bidCode":self.bidCode, @"price":self.changeBiddingTf.text} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        [hud1 hide:NO];
...
- (void)cancelBiddingPost {
    __weak MBProgressHUD *hud1 = [MBProgressHUD bwm_showHUDAddedTo:self.view title:kBWMMBProgressHUDMsgLoading animated:YES];
    [[NetworkHelper shareClient] POST:CANCEL_BIDDING_API parameters:@{@"phoneNumber":[LoginModel shareLoginModel].phone, @"bidCode":self.bidCode} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        responseObject = [NSJSONSerialization JSONObjectWithData:responseObject options:0 error:nil];
        [hud1 hide:NO];
