...
@interface OrderStatus200Controller ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation OrderStatus200Controller
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    OrderStatus200Cell *cell = [OrderStatus200Cell getCellWithTable:tableView indexPath:indexPath];
    ...
    [cell.photoImgButton addTarget:self action:@selector(photoEndLoadButtonAction:)
    ...
    if (self.failFlag) {
        ...
        cell.statusDescriptionLabel.text = @"如照片上传失败，可直接点击“收货完成”并将照片传给承运商客服";
    } else {
        cell.takeRegistrationBtn.hidden = YES;
        cell.failBtnTopConstant.constant = 0.0;
        cell.failBtnHeightConstant.constant = 0.0;
        cell.statusDescriptionLabel.text = @"装货完成时，请点击“收货完成拍照";
    }
    return cell;
}
...
- (void)photoEndLoadButtonAction:(CustomVerticalButton *)sender {
    ...
    [MyImagePickerManager presentPhotoTakeControllerInTarget:self finishPickingBlock:nil postUrlStr:PAIREACH_NETWORK_UPLOAD_URL paraDict:paraDict endResultBlock:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        ...
        if (!error) {
            ...
            MBProgressHUD *hud = [ProgressHUD bwm_showTitle:@"图片上传成功，卸货完成！" toView:self.view hideAfter:HUD_HIDE_TIMEINTERVAL];
