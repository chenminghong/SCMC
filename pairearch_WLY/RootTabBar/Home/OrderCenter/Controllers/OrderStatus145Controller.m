...
#import "OrderStatus145Cell.h"
#import "OrderListController.h"

@interface OrderStatus145Controller ()<UITableViewDelegate, UITableViewDataSource>

@property (strong, nonatomic) UITableView *tableView;

@end

@implementation OrderStatus145Controller
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    OrderStatus145Cell *cell = [OrderStatus145Cell getCellWithTable:tableView indexPath:indexPath];
...
- (void)photoStartLoadButtonAction:(CustomVerticalButton *)sender {
    ...
    [MyImagePickerManager presentPhotoTakeControllerInTarget:self finishPickingBlock:nil postUrlStr:PAIREACH_NETWORK_UPLOAD_URL paraDict:paraDict endResultBlock:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        if (!error) {
            ...
            MBProgressHUD *hud = [ProgressHUD bwm_showTitle:@"图片上传成功，装货完成！" toView:self.view hideAfter:HUD_HIDE_TIMEINTERVAL];
